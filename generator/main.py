...
def generate_vertical_slab_blockstates():
    ...
    for mod, slabs in vertical_slab_entries.items():
        for slab in slabs:
            if slab == "smooth_stone_vertical_slab" or "log" in slab or "wood" in slab or "hyphae" in slab or "stem" in slab or "bamboo_block" in slab or "stripped_bamboo" in slab or "pillar" in slab:
                data = {
                    "multipart": [
                        {
                            "when": { "facing": "south" },
                            "apply": { "model": f"{mod}:block/{slab}_negative", "uvlock": False, "y": 270 }
                        },
                        {
                            "when": { "facing": "east" },
                        ...
                        {
                            "when": { "facing": "north" },
                            "apply": { "model": f"{mod}:block/{slab}_positive", "uvlock": False, "y": 270 }
                        },
                        {
                            "when": { "facing": "west" },
            ...
            if block == "smooth_stone":
                ...
                json_data_positive = {
                    ...
                    "textures": {
                        ...
                        "west": f"minecraft:block/{block}_top"
                    }
                }
                json_data_negative = {
                    "parent": f"sloves:block/vertical_slab_negative",
